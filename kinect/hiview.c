...
void DrawVideoScene()
{
  ...
  if (current_format == FREENECT_VIDEO_RGB || current_format == FREENECT_VIDEO_YUV_RGB) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 1, frame_mode.width, frame_mode.height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
rgb_front);
  }
  glBegin(GL_TRIANGLE_FAN);
  glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
...
void mousePress(int button, int state, int x, int y)
{
  if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
    ...
    // need to get frame width&height
    freenect_frame_mode frame_mode = freenect_get_current_video_mode(f_dev);
    int xPixel = (int)((float)x/(float)frame_mode.width*640.f); 
    int yPixel = (int)((float)y/(float)frame_mode.height*480.f); 
    ...
    //int depthVal = (int)depth[xPixel*640*yPixel]; 
    ...
    char tosend[20];
    sprintf(tosend, "");
    sprintf(tosend, "x=%d&y=%d",xPixel,yPixel);
    ...
    if(curl) {
      /* First set the URL that is about to receive our POST. This URL can
         ...
         data. */ 
      curl_easy_setopt(curl, CURLOPT_URL, "http://0.0.0.0:5000");
